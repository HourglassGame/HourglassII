...
struct RunGameResultVisitor {
    typedef variant<WindowClosed_tag, GameAborted_tag, GameWon_tag, ReloadLevel_tag, move_function<std::vector<hg::InputList>()>>result_type;
    template<typename Tag>
    variant<WindowClosed_tag, GameAborted_tag, GameWon_tag, ReloadLevel_tag, move_function<std::vector<hg::InputList>()>>  operator()(Tag t) const {
        return std::move(t);
    }
};
static variant<WindowClosed_tag, GameAborted_tag, GameWon_tag, ReloadLevel_tag, move_function<std::vector<hg::InputList>()>>
       loadAndRunLevel(hg::RenderWindow &window, hg::move_function<hg::LoadedLevel(hg::OperationInterrupter &)> const& levelLoadingFunction, hg::move_function<std::vector<InputList>()>&& replayLoadingFunction = {})
{
    ...
    struct {
        ...
        variant<WindowClosed_tag, GameAborted_tag, GameWon_tag, ReloadLevel_tag, move_function<std::vector<hg::InputList>()>> operator()(WindowClosed_tag) const {
            return WindowClosed_tag{};
        }
        variant<WindowClosed_tag, GameAborted_tag, GameWon_tag, ReloadLevel_tag, move_function<std::vector<hg::InputList>()>> operator()(LoadingCanceled_tag) const {
...
int run_hourglassii() {
    ...
    while (true) {
        variant<WindowClosed_tag, RunALevel_tag, RunAReplay_tag, Exit_tag> main_menu_result = run_main_menu(window);
        std::vector<hg::InputList> replay;
        if (main_menu_result.active<WindowClosed_tag>() || main_menu_result.active<Exit_tag>()) {
            return EXIT_SUCCESS;
        }
        ...
        variant<hg::move_function<hg::LoadedLevel(hg::OperationInterrupter &)>, WindowClosed_tag, SceneAborted_tag> selected_level
            = run_level_selection_scene(window);
        
        if (selected_level.active<WindowClosed_tag>()) {
            return EXIT_SUCCESS;
        }
        else if (selected_level.active<SceneAborted_tag>()) {
            continue;
        }
        else {
        ...
        while (game_scene_result.active<ReloadLevel_tag>() || game_scene_result.active<move_function<std::vector<InputList>()>>()) {
            try {
                ...
                if (game_scene_result.active<WindowClosed_tag>()) {
                    return EXIT_SUCCESS;
                }
                else { assert(
                        game_scene_result.active<GameAborted_tag>()
