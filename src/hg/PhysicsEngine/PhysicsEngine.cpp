	...
	OperationInterrupter &interrupter) const
{
	...
	PhysicsAffectingStuff const physicsTriggerStuff(
		triggerFrameState.calculatePhysicsAffectingStuff(frame, arrivals.getList<TriggerData>(), arrivals.getList<Explosion>()));
	...
	updateExplosionsAndMakeGlitz(
		arrivals.getList<Explosion>(),
		nextExplosion,
		frame,
		ExplosionGlitzAdder(forwardsGlitz, reverseGlitz, persistentGlitz));
