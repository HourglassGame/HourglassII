	...
	std::vector<hg::InputList> const& replay)
{
	...
	bool cancelActionSent = false;
	bool hasCancelAborbingAction = false;
	...
	while (true) {
		...
		case RunState::AWAITING_INPUT:
		{
			...
			input.updateState(/*window.getInputState(), */windowglfw, mousePanel,
				getActiveButton(windowglfw), waitingForWave, cancelActionSent,
			...
			if (currentReplayIt != currentReplayEnd) {
				...
				hasCancelAborbingAction = input.getInCancelAbsorbingState();
