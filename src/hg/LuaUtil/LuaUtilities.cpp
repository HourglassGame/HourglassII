...
template<>
bool isValid<TimeDirection>(lua_State * const L, int const index) {
    try {
        if (!lua_isstring(L, index)) {
            return false;
        }
        ...
        add_semantic_callstack_info(e, "isValid<TimeDirection>");
        throw;
    }
}
...
Environment to<Environment>(lua_State *L, int index) {
    try
    {
        return { readField<Wall>(L, "wall", index), hg::GRAVITY}; //readField<int>(L, "gravity", index)};
    }
    catch (LuaError &e) {
...
Guy to<Guy>(lua_State* L, int index) {
    try {
        ...
        lua_pop(L, 1);
        int const supported(readField<int>(L, "supported", index));
...
BoxType to<BoxType>(lua_State * const L, int const index) {
    try {
        ...
        else if (strcmp(collisionTypeString, "light") == 0) {
            return BoxType::LIGHT;
        }
